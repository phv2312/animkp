...
def warp_images(from_points, to_points, images, output_region, interpolation_order = 1, approximate_grid=2, mode='reflect'):
    ...
    return [ndimage.map_coordinates(numpy.asarray(image), transform, order=interpolation_order, mode='reflect') for image in images] # if it's mask -> use mode='nearest
